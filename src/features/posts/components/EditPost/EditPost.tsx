...
const EditPostMutation = graphql`
  mutation EditPostMutation($id: ID!, $caption: String!) {
...
export const EditPost = ({
  ...
  const [commitEditMutation, isEditInFlight] =
    useMutation<EditPostMutation>(EditPostMutation);

  // Check if post is within 15-minute edit window
  const isWithinEditWindow = () => {
    const postDate = new Date(insertedAt);
    const now = new Date();
    const minutesElapsed = (now.getTime() - postDate.getTime()) / 1000 / 60;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isWithinEditWindow()) {
      setError("Posts can only be edited within 15 minutes of creation");
      return;
    }

    // Validate caption changed
    if (caption.trim() === currentCaption.trim()) {
      setError("New caption must be different from the current caption");
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-6">
            ...
            <div className="flex justify-between mt-2">
              ...
              {caption.trim() !== currentCaption.trim() && (
